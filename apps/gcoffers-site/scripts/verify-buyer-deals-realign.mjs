...
assert(map.includes("loading: rowOffset === 0 && columnOffset === 0 ? 'eager' : 'lazy'"), 'BuyerDealsMap must eagerly load only the center tile before failures and lazy-load surrounding tiles.')
...
assert(card.includes('offerFormRef.current?.scrollIntoView({ behavior: \'smooth\', block: \'start\' })'), 'Submit Offer must scroll the offer form anchor into view after expansion.')
