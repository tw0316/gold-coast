...
assert(explorer.includes('const county = deal.county?.trim()'), 'Deals explorer must classify county filters from the authoritative county field first.')
assert(explorer.includes("locationLabel.includes('palm beach')"), 'Deals explorer location-label fallback must require Palm Beach, not any Palm substring.')
