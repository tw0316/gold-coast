...
assert(explorer.includes('scrollPendingDealIdRef.current = null'), 'Deals explorer must clear pending map-scroll state when filters remove the selected deal.')
...
assert(flatMap.includes('const alreadyAttempted = current.attempted.has(tileKey) const alreadyFailed = current.failed.has(tileKey) if (alreadyAttempted && alreadyFailed) { return current } const attempted = new Set(current.attempted)'), 'BuyerDealsMap failed-tile handler must avoid Set allocations on duplicate error events.')
assert(flatMap.includes('const alreadyAttempted = current.attempted.has(tileKey) const failedHasTile = current.failed.has(tileKey) if (alreadyAttempted && !failedHasTile) { return current } const attempted = new Set(current.attempted)'), 'BuyerDealsMap tile-load handler must avoid Set allocations on duplicate load events.')
...
assert(!map.includes('aria-label={`${formatMapPrice(deal)} at ${deal.exactAddress'), 'BuyerDealsMap pin labels must use sanitizer-gated deal.locationLabel instead of directly referencing deal.exactAddress.')
...
assert(explorer.includes('onDealSelect={handleMapDealSelect}'), 'Deals explorer must route map-pin selection through the scroll-pending guard.')
